...
const DarkModePanel = ( {margins} ) => {
    const { isDarkMode, handleChangeOnInput, handleClick, value } = useContext(DarkModeContext)
  ...
  return (
    <div className={`${isDarkMode ? "bg-veryDarkGray" : "bg-lightGray" } cursor-pointer flex items-center justify-center w-[235px] xl:w-[260px] h-12 rounded-lg gap-4 ${margins}`}> 
        ...
        <input type="range" min="1" max="2" value={value} className="w-8 h-8" onChange={(e) => {handleChangeOnInput(e)}} onClick={handleClick}/>

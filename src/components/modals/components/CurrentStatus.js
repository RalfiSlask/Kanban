...
const CurrentStatus = ({ input, text, margin }) => {
  ...
  return (
    <div className={`${margin} flex flex-col gap-[8px] relative`} onClick={handleClick}>
      ...
      <div
        className={`${
          isClicked ? 'border-[#635FC7]' : 'border-inputBorder'
        } cursor-pointer flex justify-between items-center px-4 rounded border border-solid w-[100%] h-10 border-[rgba(130, 143, 163, 0.25)]`}
      >
        <p>{statusInput}</p>
